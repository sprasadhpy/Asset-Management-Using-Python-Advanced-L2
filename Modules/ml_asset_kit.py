...
def path_to_data_folder():
    return "/Users/r.shyaamprasadh/Downloads/Python and Machine Learning for Asset  Management/data/" 
...
def best_subset_regression(dep_var, exp_vars, max_vars=3):
    ...
    def best_subset(x, y, l_0):
        ...
        best_subset_prob.solve(solver=cp.ECOS_BB)
...
def plot_regime_color(data, lambda_value=0.1, figx=9, figy=6):
    ...
    '''
    # compute returns
    rets = mod1.compute_returns(data).dropna()
...
def efficient_frontier_two_regimes(rets, rets_g, rets_c, periods_per_year=12, n_ports=20, n_scenarios=10000, figx=8, figy=6):
    ...
    ann_rets = mod1.annualize_rets(rets, periods_per_year=periods_per_year)
    cov_rets = rets.cov()
    ...
    for r in rets_range:
        ww = mod1.minimize_volatility(ann_rets, cov_rets, target_return=r)
        ret_single.append( mod1.portfolio_return(ww, ann_rets) )
        vol_single.append( mod1.annualize_vol( mod1.portfolio_volatility(ww,cov_rets), periods_per_year=periods_per_year) )
    ...
    # annual returns
    ann_rets_g = mod1.annualize_rets(rets_g, periods_per_year=periods_per_year)
    ann_rets_c = mod1.annualize_rets(rets_c, periods_per_year=periods_per_year)
    ...
    vol_two, ret_two = [], []
    for r in rets_range:
        ww = mod1.minimize_volatility(ann_rets_two, cov_rets_two, target_return=r)
        ret_two.append( mod1.portfolio_return(ww, ann_rets_two) )
        vol_two.append( mod1.portfolio_volatility(ww,cov_rets_two) )
...
def regime_based_simulated_rets(rets, rets_g, rets_c, regime, periods_per_years=12, n_years=50, n_scenarios=1000):
    ...
    '''
    # compute returns
    #rets_all = mod1.compute_returns( data[assets_name] ).dropna()
